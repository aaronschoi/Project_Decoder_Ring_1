    ...
    input
      ...
      .map(char => {
        if(char === " "){
          ...
          const formatedChar = char.toLowerCase();
          for(let number in polybiusKey.horizontal){
            if(polybiusKey["horizontal"][number].includes(formatedChar)){
              for(let num in polybiusKey.vertical){
                if(polybiusKey["vertical"][num].includes(formatedChar)){
                  return num + number;
                };
              };
            };
  ...
  const decoder = input => input
    ...
    .map(char => {
        if(char === " "){
           ...
           let lookUpIndex = parseInt(indID[1]) -1;
           if(indID[0] === "4"){
               if(lookUpIndex > 1){
                    lookUpIndex += 1;
               }
                return polybiusKey.vertical[indID[0]][lookUpIndex];
           }
            else{
                return polybiusKey.vertical[indID[0]][lookUpIndex];
           };
